...
public class SimulatedFiducialTargetCamera extends SimulatedCamera
{
     ...
     */
    public SimulatedFiducialTargetCamera(String name,
                                         int colIndex,
    ...
    @Override public void updateCameraInfo()
    {
        ...
        if (camResult.hasTargets())
        {
            ...
            input.latestTargetRotation =
                target.map(it -> new TargetRotation(new Rotation3d(0, it.getPitch(), it.getYaw())))
    ...
    @Override public boolean hasValidTarget()
    {
    ...
    /**
     * Updates the target information for this camera with the information from the given fiducial
     * ID. This can be used to update the camera's target information when the camera is not able to
     * locate a target.
     *
    ...
    public void updateTargetInfoToFiducial(int fiducialId)
    {
        ...
        input.latestTargetRotation = new TargetRotation(new Rotation3d(
     ...
     */
    @Override public double getDistanceToTarget(double targetHeight)
    {
        return poseSupplier.get().getTranslation().getDistance(
            input.latestTargetPose.getTranslation().toTranslation2d());
    }
}
