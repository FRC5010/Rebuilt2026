...
public abstract class GenericCamera implements PoseProvider
{
    ...
    public GenericCamera(String name, int colIndex, Transform3d robotToCamera)
    {
        ...
        visionLayout       = visionTab.getLayout("Camera " + this.name, BuiltInLayouts.kGrid)
                           .withSize(2, 5)
    ...
    /**
     * Returns the transformation matrix from the robot's coordinate system to the camera's
     * coordinate system.
     *
     * @return the transformation matrix from the robot's coordinate system to the camera's
     *     coordinate system
     */
    ...
    public double getDistanceToTarget(double targetHeight)
    {
        ...
        return hasValidTarget() ? (targetHeight - camera2Robot.getTranslation().getZ())
                                          / (Math.tan(Math.toRadians(getTargetPitch())
                                                      + camera2Robot.getRotation().getY())
