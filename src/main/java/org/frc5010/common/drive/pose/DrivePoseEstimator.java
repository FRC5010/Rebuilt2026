...
public class DrivePoseEstimator extends GenericSubsystem
{
    ...
    public DrivePoseEstimator(GenericPose poseTracker)
    {
        ...
        DashBoard.display("Pose Degrees",
                          () -> getCurrentPose().getRotation().getMeasure().toShortString());
        ...
        tab.addStringArray("Providers Active", () -> {
               ...
               {
                   providerActive[i] = poseProviders.get(i).getClass().getSimpleName() + ": "
    ...
    {
        return PoseProviderAutoOffset.createPoseProviderAutoOffset(this::getCurrentPose, drivetrain,
                                                                   new Rotation2d());
    }
    ...
    public double[] getCurrentPose3dArray()
    {
        ...
        return new double[] {pose.getX(),     pose.getY(),     pose.getZ(),    rotation.getW(),
     ...
     *
     * <p>DISABLED_FIELD - reads pose from AT camera providers and updates the Env providers with
     * low ambiguity poses from a distance DISABLED_ENV - reads pose from Env providers, expecting a
     * pose reset to be provided ENABLED_FIELD - reads pose from the AT camera providers and updates
     * the Env providers if the pose is high caliber and close ENABLED_ENV - reads the pose from the
     * Env providers and expects given pose to be correct ALL - reads and fuses poses from both Env
     * and Field sources, resetting Env pose based on update
     */
    protected void updatePoseObservationFromProviders()
    {
        ...
        if (!disableVisionUpdateCommand)
        {
            for (PoseProvider provider : poseProviders.stream()
                                             .filter(it
                                                     -> it.isConnected()
                                                            && (state.type == ProviderType.ALL
                                                                || it.getType() == state.type))
                ...
                for (PoseObservation observation : observations)
                {
                    boolean rejectPose =
                        ...
                        || observation.pose().getX()
                               > AprilTags.aprilTagFieldLayout.getFieldLength()
                        || observation.pose().getY() < 0.0
                        || observation.pose().getY()
                               > AprilTags.aprilTagFieldLayout.getFieldWidth();
                    ...
                    if (!rejectPose)
                    {
                        ...
                        poseTracker.getVisionConsumer().accept(
                            robotPose.toPose2d(), observation.timestamp(),
                    ...
                    poseAcceptable |= activateAcceptorUpdates
                                   ...
                                   && (DriverStation.isDisabled()
                                       || (!DriverStation.isDisabled()
                                           && robotPose.getTranslation().getDistance(
                                                  getCurrentPose3d().getTranslation())
                                                  < 0.1));
                }
            }
        }
    ...
    {
        Pose3d targetPose = AprilTags.aprilTagFieldLayout.getTagPose(getClosestTagToRobot())
