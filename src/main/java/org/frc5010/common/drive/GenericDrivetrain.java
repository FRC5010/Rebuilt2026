...
public abstract class GenericDrivetrain extends GenericSubsystem
{
    ...
    public Supplier<Double> maxForwardAcceleration, maxBackwardAcceleration, maxLeftAcceleration,
        maxRightAcceleration;
    public Supplier<Double> maxForwardVelocity, maxBackwardVelocity, maxLeftVelocity,
        maxRightVelocity;
    ...
    public GenericDrivetrain(LoggedMechanism2d mechVisual)
    {
        ...
        catch (Exception e)
        {
            ...
            ppRobotConfig =
                new RobotConfig(Kilogram.of(68).magnitude(),
                                SingleJointedArmSim.estimateMOI(0.5, Kilogram.of(68).magnitude()),
                                new ModuleConfig(0.1, 4.5, 1.19, DCMotor.getNEO(1), 40, 4), 0.5);
        }
     ...
     */
    public abstract void driveWithFeedforwards(ChassisSpeeds direction,
    ...
    public void resetOrientation()
    {
        poseEstimator.resetToPose(
            new Pose2d(poseEstimator.getCurrentPose().getTranslation(),
    ...
    public Command createDefaultCommand(Controller driver)
    {
        return new DefaultDriveCommand(
            ...
            ()
                -> driver.getLeftXAxis(),
            () -> driver.getRightXAxis(), () -> isFieldOrientedDrive.getValue());
    }
    ...
    @Override public void simulationPeriodic()
    {
        ...
        int count         = 0;
        List<Pose3d> gpas = SimulatedArena.getInstance()
                                .getGamePiecesByType(Constants.Simulation.gamePieceA)
                                .stream()
                                .sorted(Comparator.comparingDouble(
                                    it -> it.getPose3d().getX() + it.getPose3d().getY()))
                                .map(it -> it.getPose3d())
        ...
        count             = 0;
        List<Pose3d> gpbs = SimulatedArena.getInstance()
                                .getGamePiecesByType(Constants.Simulation.gamePieceB)
                                .stream()
                                .map(it -> it.getPose3d())
                                .collect(Collectors.toList());
        for (Pose3d gpb : gpbs)
    ...
    protected void initializeSimulation(GenericDrivetrainConstants constants)
    {
        ...
        if (Constants.Simulation.loadSimulatedField && RobotBase.isSimulation())
        {
            ...
            int count = 0;
            for (Pose3d gpa : SimulatedArena.getInstance()
                                  .getGamePiecesByType(Constants.Simulation.gamePieceA)
                                  .stream()
                                  .sorted(Comparator.comparingDouble(
                                      it -> it.getPose3d().getX() + it.getPose3d().getY()))
            ...
            count = 0;
            for (Pose3d gpb : SimulatedArena.getInstance()
                                  .getGamePiecesByType(Constants.Simulation.gamePieceB)
                                  .stream()
                                  .map(it -> it.getPose3d())
                                  .collect(Collectors.toList()))
    ...
    public void setVelocitySuppliers(Supplier<Double> maxForwardVelocity,
    ...
    public void setUpCircularObstacle(Pose2d obstaclePosition,
        ...
        if (issueCheckCycles > 10)
        {
            ...
            boolean positionOk =
                ...
                || (currTranslation.getX() >= lowLimit && currTranslation.getY() >= lowLimit)
                       && (currTranslation.getX() <= highXLimit
                           && currTranslation.getY() <= highYLimit)
     ...
     *
     * @return an optional maple-sim {@link SwerveDriveSimulation} object, or {@link
     *     Optional#empty()} when calling from a real robot
     */
