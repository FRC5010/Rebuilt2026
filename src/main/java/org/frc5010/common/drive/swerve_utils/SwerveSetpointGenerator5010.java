...
public class SwerveSetpointGenerator5010
{
                                           ...
                                           double inputVoltage)
    {
        ...
        if (constraints != null)
        {
            ...
            desiredStateRobotRelative =
                new ChassisSpeeds(vel.getX(), vel.getY(),
            ...
            && !epsilonEquals(desiredStateRobotRelative, new ChassisSpeeds()))
        {
            ...
            // complement of the desired angle, and accelerate again.
            return generateSetpoint(prevSetpoint, new ChassisSpeeds(), constraints, dt,
                                    inputVoltage);
        }
        // Compute the deltas between start and goal. We can then interpolate from the start state
        // to the goal state; then find the amount we can move from start towards goal in this cycle
        // such that no kinematic limit is exceeded.
        ...
        // angle to command (since inverse kinematics doesn't care about angle, we can be
        ...
        // at the current angle, and then backing out the maximum interpolant between start and goal
        ...
        for (int m = 0; m < config.numModules; m++)
        {
            ...
            if (epsilonEquals(prevSetpoint.moduleStates()[m].speedMetersPerSecond, 0.0))
            {
                ...
                var necessaryRotation = prevSetpoint.moduleStates()[m].angle.unaryMinus().rotateBy(
                ...
                else
                {
                    ...
                    overrideSteering.set(
                        m, Optional.of(
                               prevSetpoint.moduleStates()[m].angle.rotateBy(Rotation2d.fromRadians(
            ...
            double s = findSteeringMaxS(prev_vx[m], prev_vy[m], prev_heading[m].getRadians(),
                                        desired_vx[m], desired_vy[m],
                                        desired_heading[m].getRadians(), max_theta_step);
        ...
        for (int m = 0; m < config.numModules; m++)
        {
            ...
            double reverseCurrentDraw = Math.abs(config.moduleConfig.driveMotor.getCurrent(
        ...
        for (int m = 0; m < config.numModules; m++)
        {
            ...
            // because we already know we can't go faster than that.
    ...
    {
        return generateSetpoint(prevSetpoint, desiredStateRobotRelative, constraints,
                                dt.in(Seconds), inputVoltage.in(Volts));
    }
    ...
    {
        return generateSetpoint(prevSetpoint, desiredStateRobotRelative, constraints,
                                dt.in(Seconds), RobotController.getBatteryVoltage());
    }
    ...
    private static double findSteeringMaxS(double x_0,
                                           double y_0,
                                           double theta_0,
                                           double x_1,
                                           double y_1,
    ...
    private static double findDriveMaxS(double x_0,
                                        double y_0,
                                        double x_1,
                                        double y_1,
                                        double max_vel_step)
