...
public class GenericSwerveDrivetrain extends GenericDrivetrain
{
    ...
    public GenericSwerveDrivetrain(LoggedMechanism2d mechVisual,
    ...
    @Override public void drive(ChassisSpeeds direction)
    {
        swerveDrive.driveRobotRelative(direction);
    }

    @Override public void setAutoBuilder()
    {
        setupPathPlanner();
    }
    ...
    public void setupPathPlanner()
    {
        AutoBuilder.configure(
            ...
            poseEstimator::resetToPose, // Method to reset odometry (will be called if your auto has
                                        // a starting
            // pose)
    ...
    @Override public void addAutoCommands(LoggedDashboardChooser<Command> selectableCommand)
    {
        swerveDrive.addAutoCommands(selectableCommand, this);
    }

    @Override public void updateGlassWidget()
    {
        ...
        for (int moduleKey = 0; moduleKey < modules.length; moduleKey++)
        {
            ...
            motorDials.get(moduleKey).setLength(
                0.1 * modules[moduleKey].steerVelocityDegreesPerSecond() + 0.02);
            expectDials.get(moduleKey).setLength(
                0.1 * modules[moduleKey].driveVelocityMetersPerSecond() + 0.02);
    ...
    @Override protected ChassisSpeeds getChassisSpeeds()
    {
        return swerveDrive.getRobotVelocity();
    }

    @Override public void initGlassWidget(GenericDrivetrainConstants constants)
    {
        SmartDashboard.putData("Drive Visual", mechanismSimulation);
        GenericSwerveModuleInfo[] modules = swerveDrive.getModulesInfo();

        visualRoots.put(
            0, mechanismSimulation.getRoot(
                   "frontleft",
        ...
        {
            visualRoots.get(i).append(new LoggedMechanismLigament2d(i + "-vert", 0.10, 90, 6.0,
                                                                    new Color8Bit(50, 50, 50)));
            visualRoots.get(i).append(new LoggedMechanismLigament2d(i + "-hori", 0.10, 0, 6.0,
                                                                    new Color8Bit(50, 50, 50)));
            motorDials.put(i, visualRoots.get(i).append(new LoggedMechanismLigament2d(
                                  i + "-motor", 0.10, 90, 6.0, new Color8Bit(Color.kYellow))));
            absEncDials.put(i, visualRoots.get(i).append(new LoggedMechanismLigament2d(
                                   i + "-Abs", 0.50, 90, 6, new Color8Bit(Color.kBlue))));
            expectDials.put(i, visualRoots.get(i).append(new LoggedMechanismLigament2d(
    ...
    public void setChassisSpeedsWithAngleSupplier(ChassisSpeeds chassisSpeeds,
                                                  DriveFeedforwards moduleFeedForwards)
    {
        ChassisSpeeds angleSuppliedChassisSpeeds =
            new ChassisSpeeds(chassisSpeeds.vxMetersPerSecond, chassisSpeeds.vyMetersPerSecond,
                              null != angleSpeedSupplier ? angleSpeedSupplier.getAsDouble()
    ...
    public Command driveToPose(Pose2d pose)
    {
        ...
        return AutoBuilder.pathfindToPose(pose, constraints, 0.0 // Goal end velocity in meters/sec
        );
    }
                                                ...
                                                Time timeout)
    {
        ...
        Supplier<DriveToPoseSupplier> finishDriving = ()
            -> new DriveToPoseSupplier(this, this::getPose,
                                       () -> pose3D.get().toPose2d(), new Transform2d(), 4.3)
                   .withInitialVelocity(() -> getFieldVelocity()); // TO-DO: Change to
        // Distance-Based PID+

        // Create the constraints to use while pathfinding
        PathConstraints constraints = new PathConstraints(
            getSwerveConstants().getkTeleDriveMaxSpeedMetersPerSecond() * 1.0, 3.5,
            getSwerveConstants().getkTeleDriveMaxAngularSpeedRadiansPerSecond(),
            getSwerveConstants().getkTeleDriveMaxAngularAccelerationUnitsPerSecond());

        return ()
                   -> new PathFinderCommand(
                          pose.get().transformBy(PathPlanOffset), constraints, 0.0, this::getPose,
                          this::getFieldVelocity, this::setChassisSpeedsWithAngleSupplier,
                          new PPHolonomicDriveController( // PPHolonomicController is the built in
                                                          // path following controller for holonomic
                                                          // drive trains
                              new PIDConstants(2.0, 0, 0.0), // Translation PID constants
                              new PIDConstants(1.0, 0, 0.0) // Rotation PID constants
                              ),
                          ppRobotConfigSupplier.get(), this)
                          .beforeStarting(() -> {
                          ...
                          // unavoidableVertices,
                          // getSwerveConstants().getTrackWidth(),
                          // getSwerveConstants().getWheelBase()))
    ...
    public Supplier<Command> driveToPoseAuton(Supplier<Pose2d> pose,
                                              Transform2d PathPlanOffset,
                                              Time timeout)
    {
        PathConstraints constraints = new PathConstraints(
            getSwerveConstants().getkTeleDriveMaxSpeedMetersPerSecond() * 1.0, 8.0,
            getSwerveConstants().getkTeleDriveMaxAngularSpeedRadiansPerSecond(),
            getSwerveConstants().getkTeleDriveMaxAngularAccelerationUnitsPerSecond());
        return driveToPoseAuton(pose, PathPlanOffset, timeout, constraints);
    }

    public Supplier<Command> newDriveToPoseAuton(Supplier<Pose2d> pose,
                                                 Transform2d PathPlanOffset,
                                                 Time timeout,
                                              ...
                                              PathConstraints driveConstraints)
    {
        ...
        return ()
                   -> new PathFinderCommand(
                          pose.get().transformBy(PathPlanOffset), driveConstraints, 0.0,
                          this::getPose, this::getFieldVelocity,
                          this::setChassisSpeedsWithAngleSupplier,
                          new PPHolonomicDriveController( // PPHolonomicController is the built in
                                                          // path following controller for holonomic
                                                          // drive trains
                                                 ...
                                                 DoubleSupplier stoppingDistance)
    {
        ...
        return ()
                   -> new PathFinderCommand(
                          ...
                          .until(()
                                     -> Math.abs(stoppingDistance.getAsDouble()
    ...
    public ChassisSpeeds getFieldVelocitiesFromJoystick(DoubleSupplier xSpdFunction,
                                                        ...
                                                        DoubleSupplier turnSpdFunction)
    {
        ...
        if (Double.isNaN(xInput) || Double.isNaN(yInput)
    ...
    {
        SwerveSetpointGenerator5010 setpointGenerator = new SwerveSetpointGenerator5010(
        ...
        PathConstraints5010 constraints = new PathConstraints5010(
            ...
            (Supplier<LinearVelocity>) ()
                -> MetersPerSecond.of(maxForwardVelocity.get()),
            (Supplier<LinearVelocity>) ()
                -> MetersPerSecond.of(maxBackwardVelocity.get()),
            (Supplier<LinearVelocity>) ()
                -> MetersPerSecond.of(maxLeftVelocity.get()),
            (Supplier<LinearVelocity>) ()
                -> MetersPerSecond.of(maxRightVelocity.get()),
            RadiansPerSecond.of(
                getSwerveConstants().getkTeleDriveMaxAngularSpeedRadiansPerSecond()),
        ...
        return startRun(
            ()
                ...
                prevSetpoint.set(new SwerveSetpoint(
                    swerveDrive.getRobotVelocity(), swerveDrive.getStates(),
                    DriveFeedforwards.zeros(swerveDrive.getModulesInfo().length)));
            },
            () -> {
                double newTime             = Timer.getFPGATimestamp();
                SwerveSetpoint newSetpoint = setpointGenerator.generateSetpoint(
                    prevSetpoint.get(), robotRelativeChassisSpeed.get(), constraints,
