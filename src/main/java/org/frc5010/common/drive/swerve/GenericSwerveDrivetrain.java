...
public class GenericSwerveDrivetrain extends GenericDrivetrain {
  ...
  public void setupBLine() {
    ...
    pathBuilder = new FollowPath.Builder(
        ...
        new PIDController(0.2, 0.0, 0.0)).withDefaultShouldFlip()
  ...
  /**
   * Schedules the PathPlanner pathfinding warmup command. Safe to call during
   * robotInit() because
   * the command is decorated with ignoringDisable(true), which lets it run while
   * the robot is
   * disabled. Calling this early prevents the ~180 ms overrun that occurs when
   * warmup finishes
  ...
  /**
   * Gets the current field-relative chassis acceleration derived from drive motor
   * acceleration
   * signals. Returns zero if the underlying drive implementation does not support
   * acceleration
   * signals.
   *
   * @return A ChassisSpeeds object representing field-relative acceleration (m/s²
   *         components)
   */
  ...
  public Supplier<Command> driveToPosePrecise(
    ...
    Supplier<DriveToPoseSupplier> finishDriving = () -> new DriveToPoseSupplier(
