...
public class AkitSwerveDrive extends SwerveDriveFunctions
{
    ...
    public AkitSwerveDrive(AkitSwerveConfig config,
                           GyroIO gyroIO,
                           ModuleIO flModuleIO,
                           ModuleIO frModuleIO,
                           ModuleIO blModuleIO,
                           ...
                           Consumer<Pose2d> resetSimulationPoseCallBack)
    {
        ...
        poseEstimator = new SwerveDrivePoseEstimator(kinematics, rawGyroRotation,
    ...
    @Override public SwerveModulePosition[] getModulePositions()
    {
    ...
    @Override public Field2d getField2d()
    {
        return field;
    }

    @Override public DrivePoseEstimator initializePoseEstimator()
    {
        return new DrivePoseEstimator(new SwerveFunctionsPose(this));
    }

    @Override public ChassisSpeeds getRobotVelocity()
    {
        return getChassisSpeeds();
    }

    @Override public ChassisSpeeds getFieldVelocity()
    {
        return ChassisSpeeds.fromRobotRelativeSpeeds(getChassisSpeeds(), getRotation());
    }

    /**
     * Returns the robot-relative chassis acceleration derived from drive motor acceleration
     * signals. Uses the same kinematics math as velocity, but with per-module acceleration instead
     * of velocity.
     */
    ...
    /**
     * Returns the field-relative chassis acceleration derived from drive motor acceleration
     * signals.
     */
    ...
    @Override public void drive(ChassisSpeeds velocity, DriveFeedforwards feedforwards)
    {
        runVelocity(velocity);
    }

    @Override public void driveFieldOriented(ChassisSpeeds velocity)
    {
        runVelocity(ChassisSpeeds.fromFieldRelativeSpeeds(velocity, gyroInputs.yawPosition));
    }

    @Override public void driveRobotRelative(ChassisSpeeds velocity)
    {
        runVelocity(velocity);
    }
    ...
    @Override public double getGyroRate()
    {
     ...
     * provided SubsystemBase and a default configuration. The default configuration includes a
     * no-op mechanism and a logger which records the state of the SysIdRoutine to the {@link
     * Logger}.
     *
        ...
        if (null == sysId)
        {
            sysId = new SysIdRoutine(
                new SysIdRoutine.Config(
                    Volts.of(0.5).per(Second), Volts.of(7), Second.of(30),
    ...
    @Override public Command sysIdDriveMotorCommand(SubsystemBase swerveSubsystem)
    {
        // Configure SysId
    ...
    @Override public Command sysIdAngleMotorCommand(SubsystemBase swerveSubsystem)
    {
    ...
    @Override
    public void drive(ChassisSpeeds robotRelativeVelocity,
    ...
    @Override public SwerveModuleState[] getStates()
    {
        return getModuleStates();
    }

    @Override public AngularVelocity getMaximumModuleAngleVelocity()
    {
        // TODO Auto-generated method stub
        throw new UnsupportedOperationException(
            "Unimplemented method 'getMaximumModuleAngleVelocity'");
    }

    @Override public GenericSwerveModuleInfo[] getModulesInfo()
    {
        if (null == moduleInfos)
     ...
     */
    @Override public Pose2d getSimPose()
    {
        return driveSimulation.getSimulatedDriveTrainPose();
    }

    @Override public Supplier<RobotConfig> getPPRobotConfigSupplier()
    {
        return robotConfigSupplier;
    }

    @Override public void setPPRobotConfigSupplier(Supplier<RobotConfig> robotConfigSupplier)
    {
                                ...
                                GenericSwerveDrivetrain drivetrain)
    {
        ...
        selectableCommand.addOption("PRO: Swerve Drive Feedforward Characterization",
                                    AkitDriveCommands.feedforwardCharacterization(
                                        drivetrain,
                                        (Voltage voltage)
                                            -> runCharacterization(voltage.in(Volts)),
                                        () -> getDriveFFCharacterizationVelocity()));
        selectableCommand.addOption("PRO: Swerve Steer Feedforward Characterization",
                                    AkitDriveCommands.feedforwardCharacterization(
                                        drivetrain,
                                        (Voltage voltage)
                                            -> runSteerCharacterization(voltage.in(Volts)),
                                        () -> getSteerFFCharacterizationVelocity()));

        selectableCommand.addOption("PRO: Swerve Angle PID Tuning",
                                    AkitDriveCommands.steerPIDTuning(drivetrain, this));
        selectableCommand.addOption("PRO: Swerve Drive PID Tuning",
                                    AkitDriveCommands.drivePIDTuning(drivetrain, this));
    }
}
