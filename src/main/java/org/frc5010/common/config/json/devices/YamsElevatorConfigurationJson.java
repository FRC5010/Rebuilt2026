...
public class YamsElevatorConfigurationJson implements DeviceConfiguration
{
     ...
     */
    @Override public Elevator configure(SubsystemBase deviceHandler)
    {
        ...
        motorConfig
            ...
            .withFeedforward(
                new ElevatorFeedforward(motorSystemId.feedForward.s, motorSystemId.feedForward.g,
                                        motorSystemId.feedForward.v, motorSystemId.feedForward.a))
            .withSimFeedforward(
                new ElevatorFeedforward(simSystemId.feedForward.s, simSystemId.feedForward.g,
                                        simSystemId.feedForward.v, simSystemId.feedForward.a));
        ...
        Optional<SmartMotorController> smartMotor = YamsConfigCommon.configureSmartMotorController(
            motorSetup, motorConfig, controlAlgorithm, motorSystemId, simSystemId, physicalParams);
        if (smartMotor.isEmpty())
        {
            throw new RuntimeException(
                "Elevator Smart motor configuration issue. ID: " + motorSetup.canId
                + " Controller: " + motorSetup.controllerType + " Motor: " + motorSetup.motorType);
        }
        ElevatorConfig m_config =
            new ElevatorConfig(smartMotor.get())
                .withStartingHeight(UnitsParser.parseDistance(startingPosition))
                .withHardLimits(UnitsParser.parseDistance(lowerHardLimit),
