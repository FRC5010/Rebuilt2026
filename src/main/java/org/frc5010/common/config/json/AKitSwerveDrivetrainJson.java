...
public class AKitSwerveDrivetrainJson implements DrivetrainPropertiesJson
{
    ...
    @Override public void createDriveTrain(GenericRobot robot)
    {
        ...
        RobotConfig PP_CONFIG = new RobotConfig(
            config.getRobotMass(), config.getDriveInertia(),
            new ModuleConfig(
                config.FrontLeft.WheelRadius, config.getMaxDriveSpeed().in(MetersPerSecond),
                constants.wheelCOF,
                DeviceConfigReader
                    .getSimulatedMotor(constants.modules.get("frontLeft").driveMotorSetup.motorType,
                                       1)
                    .withReduction(config.FrontLeft.DriveMotorGearRatio),
                config.FrontLeft.SlipCurrent, 1),
        ...
        SwerveDriveFunctions.mapleSimConfig =
            DriveTrainSimulationConfig.Default()
                ...
                .withSwerveModule(new SwerveModuleSimulationConfig(
                    ...
                    config.getDriveGearRatio(), config.getSteerGearRatio(),
                    ...
                    KilogramSquareMeters.of(config.FrontLeft.SteerInertia), constants.wheelCOF));

        if (RobotBase.isSimulation())
        {
            SwerveDriveFunctions.driveSimulation = new SwerveDriveSimulation(
            ...
            if ("TalonFX".equals(type))
            {
                ...
                driveFunctions = new AkitSwerveDrive(
                    config, new GyroIOSim(SwerveDriveFunctions.driveSimulation.getGyroSimulation()),
                    new ModuleIOTalonFXSim(config, config.FrontLeft,
                                           SwerveDriveFunctions.driveSimulation.getModules()[0]),
                    new ModuleIOTalonFXSim(config, config.FrontRight,
                                           SwerveDriveFunctions.driveSimulation.getModules()[1]),
                    new ModuleIOTalonFXSim(config, config.BackLeft,
                                           SwerveDriveFunctions.driveSimulation.getModules()[2]),
                    new ModuleIOTalonFXSim(config, config.BackRight,
                                           SwerveDriveFunctions.driveSimulation.getModules()[3]),
        ...
        else
        {
            ...
            if ("SparkTalon".equals(type))
            {
                ...
                driveFunctions = new AkitSwerveDrive(
                    config, new GyroIOPigeon2(config),
                    ...
                    new ModuleIOSparkTalon(config, config.BackRight), (pose) -> {});
            }
            else if ("Spark".equals(type))
            {
                SparkOdometryThread.createInstance();
                driveFunctions = new AkitSwerveDrive(
                    config, new GyroIOPigeon2(config), new ModuleIOSpark(0), new ModuleIOSpark(1),
                    new ModuleIOSpark(2), new ModuleIOSpark(3), (pose) -> {});
            }
            else if ("TalonFX".equals(type))
            {
                ...
                driveFunctions = new AkitSwerveDrive(
                    ...
                    new ModuleIOTalonFXReal(config, config.BackRight), (pose) -> {});
            }
            else
            {
        ...
        Pose2d startingPoseFromJson = new Pose2d(
