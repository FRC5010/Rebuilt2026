...
public class RobotsJson
{
     ...
     *
     * <p>The current robot ID is determined by the MAC address of the robot. The robot ID is
     * matched against the robots in the robots.json file. If in simulation, the first simuation
     * robot is used. If a competition pin is set, the competition robot instance is used, else the
     * MAC address is used. The robot is then created using the class specified in the robots.json
     * file, and the robot directory is passed to the constructor of the robot instance.
     *
    ...
    public GenericRobot createRobot()
    {
        ...
        for (String robotName : robots.keySet())
        {
            ...
            if ((RobotBase.isSimulation() && robotIdentity.simulate)
                || (competitionSwitch.map(it -> it.get()).orElse(false)
                    && robotIdentity.competition)
        ...
        if (robots.containsKey(robotDirectory))
        {
            ...
            try
            {
                GenericRobot robot = Class.forName(robotIdentity.robotClass)
        ...
        {
            System.out.println("Robot not found in robots.json ID: " + whichRobot
