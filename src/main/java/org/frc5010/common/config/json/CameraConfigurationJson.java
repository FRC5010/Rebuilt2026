...
public class CameraConfigurationJson
{
    ...
    /**
     * Sets whether to view game pieces in simulation. If true, the camera system will simulate
     * vision targets for game pieces in the arena. This can be useful for testing vision code
     * without a real camera. If false, the camera system will only process vision information for
     * real camera images.
     *
     ...
     *
     * <p>This property is set by the {@link #setViewGamePieces(boolean)} method and controls
     * whether the camera system will simulate vision targets for game pieces in the arena. If true,
     * the camera system will simulate game pieces in simulation; otherwise, it will only process
     * vision information for real camera images.
     *
     ...
     *
     * @param robot the {@link GenericRobot} instance to configure the camera for. The robot must
     *     have a drivetrain subsystem with a pose estimator if using pose-based vision modes
    ...
    public void configureCamera(GenericRobot robot)
    {
        ...
        Transform3d robotToCamera =
            new Transform3d(new Translation3d(x, y, z),
            ...
            switch (type)
            {
                case "limelight":
                {
                    ...
                    ((LimeLightCamera) camera)
                        .setAngleSupplier(((GenericDrivetrain) robot.getSubsystem("drivetrain"))
                                              .getPoseEstimator()
                    ...
                    if (!"none".equalsIgnoreCase(strategy))
                    {
                        if (targetFiducialIds.length > 0)
                        {
                            ...
                            camera = new PhotonVisionPoseCamera(
                                name, column, AprilTags.aprilTagFieldLayout, robotToCamera,
                                robot.getPoseSupplier(), targetFiducialIdList);
                        }
                        else
                        {
                            camera = new PhotonVisionPoseCamera(
        ...
        else
        {
            if (!"none".equalsIgnoreCase(strategy))
            {
                if (targetFiducialIds.length > 0)
                {
                    ...
                    camera = new SimulatedCamera(name, column, AprilTags.aprilTagFieldLayout,
                ...
                {
                    camera = new SimulatedCamera(name, column, AprilTags.aprilTagFieldLayout,
                                                 robotToCamera, robot.getSimulatedPoseSupplier(),
                                                 width, height, fov);
                }
            }
            else if (targetFiducialIds.length > 0)
            {
                ...
                camera = new SimulatedFiducialTargetCamera(
                    name, column, AprilTags.aprilTagFieldLayout, robotToCamera,
                    robot.getSimulatedPoseSupplier(), targetFiducialIdList, width, height, fov);
            }
            else if (targetHeight > 0)
            {
            ...
            {
                camera =
                    new SimulatedCamera(name, column, AprilTags.aprilTagFieldLayout, robotToCamera,
                                        robot.getSimulatedPoseSupplier(), width, height, fov);
            }
        }
        if (null != camera)
        ...
        switch (use)
        {
            case "target":
            {
                if (targetFiducialIds.length > 0)
                {
                    robot.addSubsystem(name,
                                       new FiducialTargetSystem((FiducialTargetCamera) camera));
                }
                else
                {
