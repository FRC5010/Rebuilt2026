...
public class RobotParser
{
     ...
     * </ol>
     *
     * <p>The configuration is not fully applied to the robot until {@link
     * #createRobot(GenericRobot)} is called.
     *
    ...
    public RobotParser(String robotDirectory, GenericRobot robot) throws IOException
    {
        ...
        // Read in the cameras
        visionJson = new ObjectMapper().readValue(new File(directory, "cameras.json"),
                                                  VisionPropertiesJson.class);
        ...
        switch (robotJson.driveType)
        {
            ...
            {
                YAGSLDrivetrainJson yagslDriveTrainJson = new ObjectMapper().readValue(
            ...
            case "AKIT_SWERVE_DRIVE":
            {
                AKitSwerveDrivetrainJson akitDriveTrainJson =
                    new ObjectMapper().readValue(new File(directory, "akit_swerve_drivetrain.json"),
     ...
     *
     * <p>This method instantiates and configures all robot subsystems based on the JSON
     * configuration files that were loaded during construction. It performs:
     *
     * <ul>
