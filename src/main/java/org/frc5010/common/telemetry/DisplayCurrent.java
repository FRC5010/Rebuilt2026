...
public class DisplayCurrent extends DisplayableValue
{
     ...
     */
    public DisplayCurrent(final double current,
     ...
     * @param logLevel - log level
     */
    public DisplayCurrent(final Current current,
                          final String name,
                          final String table,
                          LogLevel logLevel)
    {
        ...
        if (LogLevel.CONFIG == logLevel)
        {
            ...
            if (DisplayValuesHelper.isAtLogLevel(LogLevel.CONFIG))
            {
                ...
                listenerHandle_ = NetworkTableInstance.getDefault().addListener(
                    ...
                    event -> { setCurrent(event.valueData.value.getDouble(), unit_, false); });
    ...
    /**
     * Registers a listener to the display values helper to update this current with the result of
     * the given supplier when the listener is called.
     *
