...
public class DisplayAngle extends DisplayableValue
{
     ...
     * @param table - name of the table
     */
    public DisplayAngle(final double angle,
                        AngleUnit unit,
                        final String name,
                        final String table,
                        LogLevel logLevel)
    ...
    /**
     * Initializes the display by adding a listener to the subscriber and setting the default value
     * of the publisher
     */
        ...
        if (LogLevel.CONFIG == logLevel)
        {
            ...
            if (DisplayValuesHelper.isAtLogLevel(LogLevel.CONFIG))
            {
                ...
                listenerHandle_ = NetworkTableInstance.getDefault().addListener(
                    ...
                    event -> { setAngle(event.valueData.value.getDouble(), unit_, false); });
