...
public class SystemIdentification
{
    ...
    public static SysIdRoutine rpmSysIdRoutine(GenericMotorController motor,
    ...
    public static SysIdRoutine rpmSysIdRoutine(SmartMotorController motor,
                                               String motorName,
                                               SubsystemBase subsystemBase)
    {
        return new SysIdRoutine(
    ...
    public static SysIdRoutine angleSysIdRoutine(SmartMotorController motor,
                                                 String motorName,
                                                 SubsystemBase subsystemBase)
    {
        return new SysIdRoutine(
    ...
    public static SysIdRoutine angleSysIdRoutine(GenericMotorController motor,
                                                 GenericEncoder encoder,
                                                 String motorName,
                                                 SubsystemBase subsystemBase)
    {
                ...
                -> {
                    log.motor(motorName)
                        .voltage(m_appliedVoltage.mut_replace(
                            motor.get() * RobotController.getBatteryVoltage(), Volts))
                        .angularPosition(
                            m_anglePosition.mut_replace(encoder.getPosition(), Degrees))
                        .angularVelocity(
    ...
    public static Command getSysIdQuasistatic(SysIdRoutine routine,
                                                      ...
                                                      Supplier<Double> velocitySupplier)
    {
        ...
        return Commands.sequence(
            ...
            Commands
                ...
                .finallyDo(() -> {
                    double[] coefficients = computeFeedforwardCoefficients(
    ...
    static double[] computeFeedforwardCoefficients(List<Double> velocitySamples,
                                                   List<Double> voltageSamples,
                                                   List<Double> timeSamples)
    {
        ...
        if (m >= 3)
        {
            double[][] a      = {{s00, s01, s02}, {s01, s11, s12}, {s02, s12, s22}};
            double[] b        = {t0, t1, t2};
    ...
    public static Command getSysIdFullCommand(SysIdRoutine routine,
                                              ...
                                              Runnable stopMotor)
    {
        return getSysIdQuasistaticForward(routine)
            ...
            .withTimeout(quasistaticTimeout)
            .andThen(Commands.waitSeconds(delay))
            .andThen(getSysIdQuasistaticBackward(routine)
                         .until(isAtMin)
                         .finallyDo(stopMotor)
                         .withTimeout(quasistaticTimeout))
            .andThen(Commands.waitSeconds(delay))
            .andThen(
                getSysIdDynamicForward(routine).until(isAtMax).finallyDo(stopMotor).withTimeout(
                    dynamicTimeout))
            .andThen(Commands.waitSeconds(delay))
            .andThen(
                getSysIdDynamicBackward(routine).until(isAtMin).finallyDo(stopMotor).withTimeout(
                    dynamicTimeout));
    }
}
