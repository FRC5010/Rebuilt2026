...
public class VerticalPositionControlMotor extends GenericControlledMotor
{
    ...
    public VerticalPositionControlMotor(GenericMotorController motor,
                                        String visualName,
    ...
    public VerticalPositionControlMotor setupSimulatedMotor(double gearing,
                                                            Mass mass,
                                                            Distance drumRadius,
                                                            Distance minHeight,
                                                            ...
                                                            double kG)
    {
        ...
        this.conversionRotationsToDistance.setValue((drumRadius.in(Meters) *2.0 * Math.PI)
                                                    / gearing);
    ...
    @Override
    public VerticalPositionControlMotor setVisualizer(LoggedMechanism2d visualizer,
                                                      Pose3d robotToMotor)
    {
        super.setVisualizer(visualizer, robotToMotor);
        LoggedMechanismRoot2d mechanismRoot =
            visualizer.getRoot(_visualName + "mechRoot",
        ...
        LoggedMechanismLigament2d mechanismStructure =
            new LoggedMechanismLigament2d(_visualName + "-mechanism", mechanismHeight.in(Meters),
                                          90, 5, new Color8Bit(MotorFactory.getNextVisualColor()));
        mechanismRoot.append(mechanismStructure);

        setPointRoot =
            visualizer.getRoot(_visualName + "setRoot",
                               getSimX(Meters.of(robotToMotor.getX() + setPointDisplayOffset)),
                               getSimY(Meters.of(robotToMotor.getZ())));

        displayedSetpoint =
            new LoggedMechanismLigament2d(_visualName + "-setpoint", carriageHeight.in(Meters), 90,
                                          5, new Color8Bit(MotorFactory.getNextVisualColor()));
        setPointRoot.append(displayedSetpoint);

        mechRoot = visualizer.getRoot(_visualName, getSimX(Meters.of(robotToMotor.getX())),
                                      getSimY(Meters.of(robotToMotor.getZ())));
        displayedCarriage =
            new LoggedMechanismLigament2d(_visualName + "-carriage", carriageHeight.in(Meters), 90,
                                          5, new Color8Bit(MotorFactory.getNextVisualColor()));
    ...
    @Override public GenericMotorController getMotorController()
    {
        return _motor;
    }
    ...
    {
        setReference(
            reference, controller.getControlType(),
    ...
    @Override public Voltage getFeedForward(double velocity)
    {
        ...
        if (null == elevatorFeedforward || supplyKG.isPresent()
    ...
    public Voltage getDirectionalFeedForward(int movementDirection)
    {
        ...
        Voltage ff = Volts.of(elevatorFeedforward.getKs() * Math.signum(movementDirection)
    ...
    @Override public Command getSysIdCommand(SubsystemBase subsystemBase)
    {
        return SystemIdentification.getSysIdFullCommand(
            SystemIdentification.angleSysIdRoutine(_motor, encoder, "Vertical Motor",
                                                   subsystemBase),
            4, 1, 1);
    }
}
