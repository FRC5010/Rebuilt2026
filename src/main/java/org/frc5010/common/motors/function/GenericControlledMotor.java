...
/** Add your docs here. */
public abstract class GenericControlledMotor
    extends GenericFunctionalMotor implements GenericPIDController
{
    ...
    public GenericControlledMotor(GenericMotorController motor,
                                  String visualName,
     ...
     */
    @Override protected void initiateDisplayValues()
    {
    ...
    @Override public void setTolerance(double tolerance)
    {
    ...
    @Override public void setValues(GenericPID pidValues)
    {
    ...
    @Override public void setP(double p)
    {
        kP.setValue(p);
        controller.setP(p);
    }

    @Override public void setI(double i)
    {
        kI.setValue(i);
        controller.setI(i);
    }

    @Override public void setD(double d)
    {
        kD.setValue(d);
        controller.setD(d);
    }

    @Override public void setF(double f)
    {
        kF.setValue(f);
        controller.setF(f);
    }
    ...
    @Override public void setOutputRange(double min, double max)
    {
    ...
    @Override public void setControlType(PIDControlType controlType)
    {
        this.controlType.setValue(controlType.name());
        controller.setControlType(controlType);
    }

    @Override public void setProfiledMaxVelocity(double maxVel)
    {
        this.profiledMaxVel.setValue(maxVel);
        controller.setProfiledMaxVelocity(maxVel);
    }

    @Override public void setProfiledMaxAcceleration(double maxAccel)
    {
    ...
    @Override public GenericPID getValues()
    {
    ...
    @Override public double getReference()
    {
        return reference.getValue();
    }

    @Override public PIDControlType getControlType()
    {
        return controller.getControlType();
    }

    @Override public boolean isAtTarget()
    {
    ...
    public void setOutputWithFF(double power, double ffVelocity)
    {
        ...
        double actual =
            MathUtil.clamp(power * outputFactor + ff, minOutput.getValue() * outputFactor,
                           maxOutput.getValue() * outputFactor);
        outputEffort.setVoltage(actual, Volts);
        _motor.setVoltage(actual);
    }

    @Override public double calculateControlEffort(double current)
    {
        double controlEffort = controller.calculateControlEffort(current);
        MathUtil.clamp(controlEffort, minOutput.getValue(),
    ...
    @Override public double getReferenceVelocity()
    {
        return controller.getReferenceVelocity();
    }

    @Override public void resetController(double position, double velocity)
    {
        controller.resetController(position, velocity);
    }

    @Override public void setMotorFeedFwd(MotorFeedFwdConstants feedFwd)
    {
        if (0 != feedFwd.getkS())
    ...
    {
        double feedforward =
            (null == feedFwd ? controller.getF()
                             : controller.getReference() * (feedFwd.getkV() + feedFwd.getkA())
                                   + feedFwd.getkS());
        feedForward.setValue(feedforward);
