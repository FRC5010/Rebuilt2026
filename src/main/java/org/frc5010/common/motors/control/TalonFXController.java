...
public class TalonFXController extends GenericPIDControllerAbsract
{
    ...
    /**
     * Sends a control request to the TalonFX motor based on the set control type and reference
     * point.
     *
     ...
     * absolute difference between the current position and the target position is checked against
     * the tolerance. If the motor is being controlled in duty cycle control mode, the function
     * returns false.
     *
     ...
     */
    @Override public void setTolerance(double value)
    {
        tolerance = value;
    }
     ...
     *
     * <p>This function will take the PID values and apply them to the TalonFX motor. The kP, kI,
     * kD, and kS constants are set using the {@link TalonFXConfigurator} class.
     *
     * @param pid the PID values to set.
     */
    @Override public void setValues(GenericPID pid)
    {
        ...
        if (null == this.request || this.controlType != controlType)
        {
            switch (controlType)
            {
                ...
                case PROFILED_POSITION:
                    ...
                    break;
                case PROFILED_VELOCITY:
                    request = new MotionMagicVelocityVoltage(reference).withEnableFOC(
                        motor.isFOCEnabled());
                    break;
                default:
                    throw new IllegalArgumentException("Unsupported TalonFX control type "
    ...
    @Override public void setProfiledMaxVelocity(double maxVelocity)
    {
    ...
    @Override public void setProfiledMaxAcceleration(double maxAcceleration)
    {
        cfg.refresh(configuration.MotionMagic);
    ...
    @Override public void setMotorFeedFwd(MotorFeedFwdConstants motorConstants)
    {
        cfg.refresh(configuration.Slot0);
