...
public class DriveToPoseSupplier extends GenericCommand
{
     ...
     */
    public DriveToPoseSupplier(GenericSwerveDrivetrain swerveSubsystem,
                               ...
                               double maxAcceleration)
    {
        ...
        thetaConstraints =
            new TrapezoidProfile.Constraints(maxAngularSpeed, MAX_ANGULAR_ACCELERATION);

        distanceController =
            new ProfiledPIDController(pidTranslation.getkP(), pidTranslation.getkI(),
                                      pidTranslation.getkD(), translationConstraints);
        thetaController = new ProfiledPIDController(pidRotation.getkP(), pidRotation.getkI(),
                                                    pidRotation.getkD(), thetaConstraints);
    ...
    {
        return targetPoseProvider.get().getTranslation().getDistance(
            poseProvider.get().getTranslation());
    }
    ...
    {
        return poseProvider.get().getTranslation().plus(
            currentVelocity.times(deltaTime * NUM_CYCLES));
    }
    ...
    public Rotation2d getFutureAngleToTarget()
    {
        ...
        return new Translation2d(targetPoseProvider.get().getX() - futurePose.getX(),
    ...
    {
        currentVelocity = new Translation2d(initialVelocity.get().vxMetersPerSecond,
                                            initialVelocity.get().vyMetersPerSecond);
        speedTowardsTarget =
            Math.min(0.0, -currentVelocity
                               .rotateBy(targetPoseProvider.get()
                                             .getTranslation()
    ...
    @Override public void init()
    {
        ...
        thetaController.reset(robotPose.getRotation().getRadians(),
                              initialVelocity.get().omegaRadiansPerSecond);
    ...
    @Override public void execute()
    {
        ...
        double translationalSpeed = distanceToGoal > 0.25
        ...
        lastSetpointTranslation =
            new Pose2d(
                ...
                .transformBy(new Transform2d(distanceController.getSetpoint().position, 0.0,
                                             new Rotation2d()))
                .getTranslation();
        ...
        ChassisSpeeds chassisSpeeds = ChassisSpeeds.fromFieldRelativeSpeeds(
            speedX
                + distanceController.getSetpoint().velocity * movementAngle.getCos()
                      * ffInclusionFactor,
            speedY
                + distanceController.getSetpoint().velocity * movementAngle.getSin()
                      * ffInclusionFactor,
            thetaSpeed
        ...
        SmartDashboard.putNumber("Distance Velocity Setpoint",
                                 distanceController.getSetpoint().velocity);
    ...
    @Override public void stop(boolean interrupted)
    {
        onTargetCounter = 0;
