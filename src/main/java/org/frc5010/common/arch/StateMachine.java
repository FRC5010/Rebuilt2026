 ...
 *       Command cmd1 = Commands.runOnce(()-> System.out.println("command 1 printed this one
 * line.")).ignoringDisable(true); Command cmd2 = Commands.run(()-> System.out.println("command 2
 ...
 *
 *       // Examples of a (not recommended) stop state and idle state that could have been used (but
 * were not) State stop = stateMachine.addState("stop state",
 * Commands.none().ignoringDisable(true)); // better to use "exitStateMachine"; test message State
 * idle = stateMachine.addState("idle state", Commands.idle().ignoringDisable(true)); // test
 * message
 *
...
public class StateMachine extends Command
{
        ...
        for (WeakReference<State> state : states)
        {
            ...
            for (Transition transition : state.get().transitions)
            {
                ...
                sb.append("transition ")
                    .append(transition)
                    .append(" to ")
                    .append(transition.nextState != null ? transition.nextState.name
                                                         : "exit StateMachine")
    ...
    @Override public void initialize()
    {
        ...
        events
            .clear(); // make sure clear in case there would be a race between the execute poll and
        // the next command clear (maybe used if FSM can start/stop which it can't right
        // now)
        CommandScheduler.getInstance().schedule(
            new ScheduleCommand(initialState.stateCommandAugmented));
    }
    ...
    private class WrapState extends WrapperCommand
    {
        ...
        /**
         * We're here because somebody scheduled us and we are running. The initial state was
         * scheduled when the StateMachine started. All the rest of the states that run must be
        ...
        @Override public void initialize()
        {
            ...
            if (!state.transitions.isEmpty())
            {
                ...
                { // add all the events for this state
                    var trigger =
                        new Trigger(events, transition.triggeringEvent); // for .when(condition)
                    if (transition.nextState == null)
                    { // for .exitStateMachine()
                        trigger.onTrue(
                            Commands.runOnce(() -> exitStateMachine = true).ignoringDisable(true));
                    }
                    else
                    {
                        trigger.onTrue(
                            transition.nextState
                                .stateCommandAugmented); // external triggering next state
                    }
                }
            }
            ...
            m_command.initialize(); // Wrapper is done with its fussing so tell original command to
                                    // initialize
        }
        ...
        @Override public void end(boolean interrupted)
        {
            ...
            if (state.transitions.isEmpty())
            { // no transitions [no .when() nor .whenComplete()]
                exitStateMachine = true; // no matter how this ended tell StateMachine to exit since
                                         // nowhere to go from
                // here
            }
            else
            {
                if (!interrupted)
                {
                    completedNormally = state; // for internal trigger, indicate state ended by
                                               // itself without others help
                    ...
                    { // check all transitions
                        ...
                        { // for .whenComplete()
                            ...
                            break; // don't look for any more since cannot be more than one
    ...
    {
        private final String name;
        private Command stateCommandAugmented; // the Wrapped (instrumented) state command that will
                                               // actually be run
        ...
        /**
         * A builder for a transition from one state to another. Use {@link #to(State)} to specify
         ...
         * true, or use {@link #whenComplete()} to make the transition occur when the originating
         * state completes without having reached any other transitions first.
         */
        public final class TransitionNeedsConditionStage
        {
            ...
            { // imply run until the external event then transition
                checkDuplicateCondition(condition);
                transitions.add(new Transition(
                    m_targetState,
                    condition)); // wrap condition and add to the list a transition to
                // this state
            }

            /**
             * Marks the transition when the originating state completes without having reached any
             * other transitions first.
             */
             ...
             *
             * <p>This check cannot prevent effectively identical conditions in different objects
             * from being used. The user must assure that two or more conditions in a state will not
             * trigger at the same time. The results of two identical conditions are essentially
             * undefined.
             *
                ...
                for (Transition transition : transitions)
                {
                    if (transition.triggeringEvent == condition)
                    {
                        throw new IllegalArgumentException(
                            "Condition object can be used only once per state.");
                    }
                }
            }
