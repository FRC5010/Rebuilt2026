...
public class WpiNetworkTableValuesHelper implements WpiHelperInterface
{
     ...
     * constructor before putting the class on Display The overloaded function implementations can
     * be used without needing to specify the type.
     *
     * @param key - String name of the variable being stored. Recommend using a final String
     *     constant.
    ...
    public void set(String key, String type, Object val)
    {
        ...
        if (null != valueType)
        {
            ...
            {
                log_rio("Error setting " + val + " to " + key + " which is type " + valueType
                        + " not " + type);
            }
        }
        else
            ...
            switch (value.getFirst())
            {
                ...
                case "Boolean":
                    return Boolean.valueOf(
                        Preferences.getBoolean(key, (Boolean) value.getSecond()));
            }
        }
            ...
            switch (value.getFirst())
            {
                case "Double":
                    tab.addDouble(key,
                                  ()
                                      -> Double.valueOf(
                                          Preferences.getDouble(key, (Double) value.getSecond())));
                case "Integer":
                    tab.addInteger(key,
                                   ()
                                       -> Integer.valueOf(
                                           Preferences.getInt(key, (Integer) value.getSecond())));
                case "String":
                    tab.addString(key,
                                  () -> Preferences.getString(key, (String) value.getSecond()));
                case "Boolean":
                    tab.addBoolean(key,
                                   ()
                                       -> Boolean.valueOf(Preferences.getBoolean(
                                           key, (Boolean) value.getSecond())));
            }
        }
        else if (sendables.containsKey(key))
        {
    ...
    public void addToTabList(ShuffleboardTab tab, String list, String key)
    {
        ...
        if (Preferences.containsKey(key))
        {
            ...
            {
                case "Double":
                    layout.addDouble(key,
                                     ()
                                         -> Double.valueOf(Preferences.getDouble(
                                             key, (Double) value.getSecond())));
                case "Integer":
                    layout.addInteger(key,
                                      ()
                                          -> Integer.valueOf(Preferences.getInt(
                                              key, (Integer) value.getSecond())));
                case "String":
                    layout.addString(key,
                                     () -> Preferences.getString(key, (String) value.getSecond()));
                case "Boolean":
