...
{
    /**
     * Autonomous event loop used to bind triggers which are only active while the autonomous
     * sequence is running
     */
    ...
    public Trigger robotWithinRectangularRegion(Pose2d topLeft, Pose2d bottomRight)
    {
        return autonTrigger(() -> {
         ...
         && robotPose.getY() >= bottomRight.getY() && robotPose.getY() <= topLeft.getY();
        });
    }
    ...
    /**
     * Constructs a trigger which mirrors the state of an event marker. Used with the
     * EventMarkerFlag command.
     */
