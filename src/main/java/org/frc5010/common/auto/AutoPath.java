...
public class AutoPath
{
    ...
    /**
     * gets a command which can be used to reset the robot's odometry to the start of the path if
     * the robot is far from the start
     *
    ...
    public Command resetOdometryToStart(double distanceToTrigger)
    {
        ...
        resetCommand.setName("Reset Odometry to Start of Path " + pathplannerPath.name
                             + " if far from start");
        return resetCommand;
    }

    /**
     * gets a BooleanSupplier that returns true if the robot is far from the start of the path.
     * mainly used to determine if certain auto correction procedures should be run.
     *
    ...
    {
        return ()
                   -> pathplannerPath.getStartingHolonomicPose().get().getTranslation().getDistance(
