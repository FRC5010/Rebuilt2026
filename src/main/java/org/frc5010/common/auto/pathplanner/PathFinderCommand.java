...
public class PathFinderCommand extends Command
{
     ...
     */
    public PathFinderCommand(PathPlannerPath targetPath,
     ...
     * @param requirements the subsystems required by this command
     */
    public PathFinderCommand(Pose2d targetPose,
                             PathConstraints constraints,
                             double goalEndVel,
                             Supplier<Pose2d> poseSupplier,
                             Supplier<ChassisSpeeds> speedsSupplier,
    ...
    public PathFinderCommand(Pose2d targetPose,
                             ...
                             Subsystem... requirements)
    {
        this(targetPose, constraints, goalEndVel.in(MetersPerSecond), poseSupplier, speedsSupplier,
             output, controller, robotConfig, requirements);
    }
    ...
    public PathFinderCommand(Pose2d targetPose,
                             ...
                             Subsystem... requirements)
    {
        this(targetPose, constraints, 0.0, poseSupplier, speedsSupplier, output, controller,
             robotConfig, requirements);
    }
    ...
    public static void updatePregen(PathConstraints constraints,
                                    Pose2d robotPose,
                                    ...
                                    RobotConfig config)
    {
        if (Pathfinding.isNewPathAvailable())
        {
            ...
            if (null != pathPregen)
            {
                trajPregen = new PathPlannerTrajectory(pathPregen, new ChassisSpeeds(),
                                                       robotPose.getRotation(), config);
                pregenUsed = false;
            }
        }
    }
    ...
    @Override public void initialize()
    {
        ...
        if (targetPath != null)
        {
            ...
            if (shouldFlipPath.getAsBoolean())
            {
                ...
                goalEndState =
                    new GoalEndState(goalEndState.velocityMPS(), targetPose.getRotation());
            }
        }
    ...
    @Override public void execute()
    {
        ...
        if (!skipUpdates && Pathfinding.isNewPathAvailable())
        {
            ...
            if (currentPath != null)
            {
                ...
                {
                    double closest2Dist = currentTrajectory.getState(closestState2Idx)
                                              .pose.getTranslation()
                                              .getDistance(currentPose.getTranslation());
                    double nextDist = currentTrajectory.getState(closestState2Idx + 1)
                                          .pose.getTranslation()
                                          .getDistance(currentPose.getTranslation());
                ...
                double d = closestState1.pose.getTranslation().getDistance(
                    closestState2.pose.getTranslation());
                double t =
                    (currentPose.getTranslation().getDistance(closestState1.pose.getTranslation()))
                    / d;
                ...
                // If the robot is stationary and at the start of the path, set the time offset to
                // the next loop This can prevent an issue where the robot will remain stationary if
                // new paths come in every loop
                if (timeOffset <= 0.02
    ...
    @Override public void end(boolean interrupted)
    {
        ...
        // interrupting the command to smoothly transition into some auto-alignment routine
    ...
    public static Command warmupCommand()
    {
        return new PathFinderCommand(
                   new Pose2d(15.0, 4.0, Rotation2d.k180deg), new PathConstraints(4, 3, 4, 4),
                   ()
                   ...
                   new RobotConfig(
                       75, 6.8,
                       new ModuleConfig(0.048, 5.0, 1.2,
                                        DCMotor.getKrakenX60(1).withReduction(6.14), 60.0, 1),
                       0.55))
