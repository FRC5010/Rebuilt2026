...
public class LEDStrip extends GenericSubsystem
{
    ...
    private LEDStrip(int kPort, int length)
    {
        ...
        segments.put(ConfigConstants.ALL_LEDS, Segment.create()
    ...
    /**
     * Sets the default pattern to run on the entire LED strip when the LED subsystem is not running
     * a command.
     *
    ...
    /**
     * Sets whether the pattern with the given name is active or not. If the name does not
     * correspond to a pattern, this method does nothing.
     *
    ...
    public static void addSegment(String name, int startIndex, int length)
    {
        ...
        AddressableLEDBufferView view = instance.m_buffer.createView(startIndex, length);
        segments.put(name,
                     Segment.create().setView(view).setPattern(LEDPattern.kOff).setActive(false));
    }
    ...
    public static void addSegment(String name, int startIndex, int length, int order)
    {
        ...
        AddressableLEDBufferView view = instance.m_buffer.createView(startIndex, length);
        segments.put(name, Segment.create()
                               .setView(view)
    ...
    /**
     * Clears all active patterns on the LED strip. This method is useful for completely clearing
     * the LED strip of all patterns before running a new command.
     */
    ...
    /**
     * Returns a new LEDPattern that sets all LEDs to a rainbow color, and scrolls the pattern at
     * the given speed.
     *
    ...
    {
        return LEDPattern.rainbow(255, 255).scrollAtRelativeSpeed(
            Percent.per(Second).of(percentScrollingSpeed));
    }
    ...
    /**
     * Returns a new LEDPattern that masks the given base pattern with a band of given visibility
     * and scrolling speed. The mask has a band of white (visible) that is percentVisible of the
     * pattern's length wide, centered at percentVisible of the pattern's length from the start of
     * the pattern. The rest of the mask is black (invisible). The resulting pattern is then
     * scrolled at the given speed.
     *
     ...
     */
    public static LEDPattern getMaskedPattern(LEDPattern basePattern,
     ...
     * @return the new masked and scrolling pattern
     */
    public static LEDPattern getBand(LEDPattern basePattern,
                                     double bandCenter,
                                     double percentWidth,
                                     double percentScrollingSpeed)
    {
        Map<Double, Color> maskSteps =
            Map.of(0.0, Color.kBlack, bandCenter - (percentWidth / 2), Color.kWhite,
                   bandCenter - (percentWidth / 2), Color.kBlack);
        LEDPattern mask = LEDPattern.steps(maskSteps).scrollAtRelativeSpeed(
            Percent.per(Second).of(percentScrollingSpeed));
        return basePattern.mask(mask);
    }
