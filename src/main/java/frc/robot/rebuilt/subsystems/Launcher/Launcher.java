...
public class Launcher extends GenericSubsystem
{
    ...
    {
        return inputs.stateCurrent == LauncherState.PREP
     || inputs.stateCurrent == LauncherState.PRESET;
    }
    ...
    @Override public Command getDefaultCommand()
    {
        return Commands.runOnce(() -> { io.stopAllMotors(); }, this);
    }
    ...
    public Command trackTargetCommand()
    {
        return Commands.run(() -> {
            ...
            io.setTurretRotationWithFeedforward(inputs.turretAngleCalculated,
    ...
    /**
     * A command which stops the tracking of a target and resets the turret rotation and hood angle
     * to 0 degrees.
     *
    ...
    {
        return inputs.flyWheelSpeedAtGoal
     || inputs.flyWheelSpeedActual.gt(inputs.flyWheelSpeedDesired);
    }
    ...
    public Command increaseFlywheelSpeedCommand()
    {
        return Commands.runOnce(() -> {
            ...
            if (newSpeed.lt(RPM.of(300)))
            { // Assuming 300 RPM as the upper limit
                io.setFlyWheelVelocity(newSpeed);
            }
        });
    }
    ...
    public Command decreaseTurretAngleCommand()
    {
        return Commands.runOnce(() -> {
            ...
            if (newAngle.gt(Degrees.of(-90)))
            { // Assuming -90 degrees as the left limit
    ...
    public Command increaseTurretAngleCommand()
    {
        return Commands.runOnce(() -> {
            ...
            if (newAngle.lt(Degrees.of(90)))
            { // Assuming 90 degrees as the right limit
                io.setTurretRotation(newAngle);
            }
        });
    }
