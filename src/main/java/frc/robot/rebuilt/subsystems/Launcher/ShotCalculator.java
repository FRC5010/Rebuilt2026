...
public class ShotCalculator
{
    ...
    private DoubleFunction<Double> settlingTimeFunction =
        TurretControlPhysics.trapezoidalSettlingTimeFunction(turretMaxVelocityRadPerSec,
                                                             turretMaxAccelRadPerSecSq);
    ...
    public record ShotTables(Map<Double, Rotation2d> hoodAngles,
                             ...
                             double phaseDelaySeconds)
    {
        ...
        {
            return new ShotTables(hoodAngles, flywheelSpeeds, timeOfFlightSeconds,
                                  minDistanceMeters, maxDistanceMeters, newPhaseDelaySeconds);
        }
    }
    ...
    public record BallisticConfig(double minDistanceMeters,
                                  double maxDistanceMeters,
    ...
    public static ShotTables createDefaultTables()
    {
        ...
        return new ShotTables(
            Map.ofEntries(Map.entry(2.0796297600462808, Rotation2d.fromDegrees(33.0)),
            ...
            Map.ofEntries(Map.entry(2.0796297600462808, 97.0), Map.entry(2.3644814757115706, 103.0),
                          Map.entry(2.7648598116063776, 105.0),
                          Map.entry(3.2194779503261004, 107.0), Map.entry(3.53088512698516, 110.0),
                          Map.entry(3.9268571046742813, 113.0), Map.entry(4.317290273504823, 115.0),
                          Map.entry(4.540307519714445, 117.0), Map.entry(5.77893560525366, 127.0),
                          Map.entry(6.35214199070158, 133.0)),
            Map.ofEntries(Map.entry(2.11, 1.04), Map.entry(3.92, 1.19), Map.entry(4.10, 1.22),
                          Map.entry(5.58, 1.28)),
            0.7, 10.0, 0.03);
    }
    ...
    public static ShotTables createBallisticTables(BallisticConfig config)
    {
        ...
        for (double distance = minDistance; distance <= maxDistance + 1e-6;
             distance += distanceStep)
        {
            BallisticSolution solution =
                solveBallistic(distance, heightDelta, gravity, minAngle, maxAngle, angleStep,
                               config.wheelRadiusMeters(), config.minFlywheelRadPerSec(),
                               config.maxFlywheelRadPerSec());
            if (solution == null)
            {
                continue;
            }
            hoodAngles.put(distance,
                           Rotation2d.fromRadians(hoodReference - solution.angleRadians()));
        ...
        return new ShotTables(hoodAngles, flywheelSpeeds, timeOfFlight, minRange, maxRange,
                              config.phaseDelaySeconds());
    }

    private record BallisticSolution(double angleRadians,
                                     double flywheelRadPerSec,
                                     double timeOfFlightSeconds)
    {
    }

    private static BallisticSolution solveBallistic(double distanceMeters,
                                                    ...
                                                    double maxFlywheelRadPerSec)
    {
        ...
        for (double angle = minAngleRadians; angle <= maxAngleRadians + 1e-6;
             angle += angleStepRadians)
        {
            ...
            double velocitySquared = gravityMetersPerSecondSquared * distanceMeters * distanceMeters
            ...
            if (flywheelRadPerSec < minFlywheelRadPerSec
                || flywheelRadPerSec > maxFlywheelRadPerSec)
            {
                continue;
            }
    ...
    public double[] getBallisticGuess(double distanceMeters)
    {
        ...
        double angleStep =
            Math.max(ballisticConfig.hoodAngleStep().getRadians(), Math.toRadians(0.25));
        double heightDelta =
            ballisticConfig.targetHeightMeters() - ballisticConfig.launchHeightMeters();
        BallisticSolution solution = solveBallistic(
            distanceMeters, heightDelta, ballisticConfig.gravityMetersPerSecondSquared(), minAngle,
            maxAngle, angleStep, ballisticConfig.wheelRadiusMeters(),
            ballisticConfig.minFlywheelRadPerSec(), ballisticConfig.maxFlywheelRadPerSec());
        if (solution == null)
        {
            return null;
        }
        double hoodAngleDegrees = Math.toDegrees(hoodReference - solution.angleRadians());
        return new double[] {hoodAngleDegrees, solution.flywheelRadPerSec(),
     ...
     */
    public void addDataPoint(double distanceMeters,
    ...
    public void setTurretConstraints(Rotation2d minAngle,
                                     Rotation2d maxAngle,
                                     Rotation2d paddingAngle)
    {
        if (minAngle != null)
        {
    ...
    /**
     * Configures the turret settling-time function using the closed-form trapezoidal motion
     * profile.
     *
     * <p>This replaces any previously supplied {@link #setSettlingTimeFunction} with the analytical
     * result derived from the given peak velocity and acceleration. Call this once during robot
     * init after measuring the turret's actual motion profile constraints via SysId.
     *
     ...
     *     oscillation)
     */
    public void setTurretMotionConstraints(double maxVelocityRadPerSec,
                                           double maxAccelRadPerSecSq,
                                           double newSettlingGain)
    {
        turretMaxVelocityRadPerSec = maxVelocityRadPerSec;
        turretMaxAccelRadPerSecSq  = maxAccelRadPerSecSq;
        settlingTimeFunction       = TurretControlPhysics.trapezoidalSettlingTimeFunction(
            turretMaxVelocityRadPerSec, turretMaxAccelRadPerSecSq);
        settlingGain         = newSettlingGain;
        turretControlPhysics = null;
    }

    public ShootingParameters getParameters(Translation2d turretRelativePosition,
                                            ...
                                            Supplier<Translation2d> targetPositionSupplier)
    {
        ...
        // Field velocity is used for linear extrapolation: x += vx*dt, y += vy*dt, heading +=
        // omega*dt. This matches the actual drivetrain behavior since the drive code already
        // accounts for curvature (discretize) when commanding inputs to drive straight in field
        // frame.
        ...
        Pose2d turretPosition = phaseDelayedPose.transformBy(
            new Transform2d(turretRelativePosition.getMeasureX(),
                            turretRelativePosition.getMeasureY(), turretRelativeAngle));

        TurretControlPhysics physics = getTurretControlPhysics(turretRelativePosition);
        TurretControlPhysics.AimingSolution solution = physics.solve(
            target, turretRelativeAngle, (timeSinceStartSeconds, lookaheadSeconds) -> {
                ...
                return new TurretControlPhysics.RobotState(predictedPose, fieldVelocity,
                                                           fieldAcceleration);
            });
        ...
        turretVelocity = turretAngleFilter.calculate(turretAngle.minus(lastTurretAngle).getRadians()
        ...
        lastHoodAngle   = hoodAngle;
        latestParameters =
            new ShootingParameters(solution.isPossible(), turretAngle, turretVelocity, hoodAngle,
                                   hoodVelocity, flywheelSpeed != null ? flywheelSpeed : 0.0,
                                   Meters.of(distanceToVirtualTarget), solution);
        ...
        Logger.recordOutput("ShotCalculator/TurretToTargetDistance",
                            solution.effectiveDistanceMeters());
        Logger.recordOutput(
        ...
        Rebuilt.drivetrain.getField2d()
            .getObject(targetName)
            .setPose(new Pose2d(target, target.getAngle()));
        // Lookahead visualisation uses the same linear extrapolation
        Pose2d lookaheadRobotPose  = linearExtrapolatePose(phaseDelayedPose, fieldVelocity,
                                                          solution.estimatedTimeOfFlight());
        Pose2d lookaheadTurretPose = lookaheadRobotPose.transformBy(
            new Transform2d(turretRelativePosition.getMeasureX(),
                            turretRelativePosition.getMeasureY(), turretRelativeAngle));
     ...
     */
    private static Pose2d linearExtrapolatePose(Pose2d currentPose,
                                                ChassisSpeeds fieldVelocity,
                                                double dt)
    {
        Translation2d newTranslation = currentPose.getTranslation().plus(new Translation2d(
            fieldVelocity.vxMetersPerSecond * dt, fieldVelocity.vyMetersPerSecond * dt));
        Rotation2d newRotation       = currentPose.getRotation().plus(
            Rotation2d.fromRadians(fieldVelocity.omegaRadiansPerSecond * dt));
    ...
    {
        if (turretControlPhysics == null || cachedTurretOffset == null
            || !cachedTurretOffset.equals(turretOffset))
        {
            cachedTurretOffset   = turretOffset;
            turretControlPhysics = new TurretControlPhysics(
                turretOffset, minTurretAngle, maxTurretAngle, feedforwardPaddingAngle, settlingGain,
                this::getTimeOfFlightSeconds, settlingTimeFunction, minDistance, maxDistance);
        }
