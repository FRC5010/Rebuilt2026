...
public class LauncherIOReal implements LauncherIO
{
    ...
    public LauncherIOReal(Map<String, Object> devices, Map<String, GenericSubsystem> subsystems)
    {
        ...
        robotToTurret = turret.getPivotConfig()
        ...
        turretHighLimit = turret.getMotorController().getConfig().getMechanismUpperLimit().orElse(
            turretHighLimit);
        ...
        easyCrt = new EasyCRTConfig(()
                      ...
                      .withCommonDriveGear(
                          ...
                          /* encoder1Pinion */ 40,
                          /* encoder2Pinion */ 36)
                      .withAbsoluteEncoderOffsets(Rotations.of(-0.244),
                                                  Rotations.of(-0.210)) // set after mechanical zero
                      .withMechanismRange(Degrees.of(-168), Degrees.of(173)) // -360 deg to +720 deg
                      .withMatchTolerance(
                          Rotations.of(0.06)) // ~1.08 deg at encoder2 for the example ratio
        ...
        // // Test Values
        SmartDashboard.putNumber("EasyCRT/Unique Coverage",
                                 easyCrt.getUniqueCoverage().orElse(Degrees.of(0.0)).in(Degrees));
        SmartDashboard.putBoolean("EasyCRT/Coverage Satisfies Range",
                                  easyCrt.coverageSatisfiesRange());
        SmartDashboard.putNumber("EasyCRT/Enc 1", easyCrt.getAbsoluteEncoder1Angle().in(Degrees));
        SmartDashboard.putNumber("EasyCRT/Enc 1 Ratio",
                                 easyCrt.getEncoder1RotationsPerMechanismRotation());
        SmartDashboard.putNumber("EasyCRT/Enc 2", easyCrt.getAbsoluteEncoder2Angle().in(Degrees));
        SmartDashboard.putNumber("EasyCRT/Enc 2 Ratio",
                                 easyCrt.getEncoder2RotationsPerMechanismRotation());
        ...
        turret.min()
            .or(turret.max())
            .onTrue(Commands.runOnce(() -> turret.getMotor().setDutyCycle(0)));
    }
        ...
        Supplier<Translation2d> targetPositionSupplier)
    {
        ...
        return ShotCalculator.getInstance().getParameters(
            robotToTurret, Rotation2d.fromDegrees(turret.getAngle().in(Degrees)), robotPoseSupplier,
    ...
    public void updateInputs(LauncherIOInputs inputs)
    {
        ...
        if (targetPose.isPresent())
        {
            ...
            ShotCalculator.ShootingParameters params = ShotCalculator.getInstance().getParameters(
                robotToTurret, Rotation2d.fromDegrees(turret.getAngle().in(Degrees)),
                ()
            ...
            inputs.targetDistance =
                Meters.of(inputs.robotToTarget.getDistance(new Translation2d()));
        }
        /** Reads the desired flywheel, hood, and turret setpoints */
        inputs.flyWheelSpeedDesired =
            flyWheel.getMotorController().getMechanismSetpointVelocity().map(it -> it).orElse(
                RPM.of(0.0));
        ...
        inputs.turretAngleError =
            inputs.turretAngleActual.minus(inputs.turretAngleDesired).in(Degrees);
    ...
    @Override public void configureShotCalculator(ShotCalculator shotCalculator)
    {
        ...
        Rotation2d aimTolerance = Rotation2d.fromDegrees(
        ...
        double maxAccelRadPerSecSq = trapConstraints.map(c -> c.maxAcceleration * 2.0 * Math.PI)
    ...
    {
        hood.getMotorController().setPosition(
            hood.getArmConfig().getLowerHardLimit().orElse(Degrees.of(30)));
        LEDStrip.changeSegmentPattern(ConfigConstants.ALL_LEDS,
     ...
     */
    @Override public void setTurretRotationWithFeedforward(Angle angle, double feedforwardRadPerSec)
    {
        if (angle.gt(turretHighLimit))
        {
            SmartDashboard.putBoolean("Launcher/Turret Limit", true);
        ...
        if (rawController instanceof com.ctre.phoenix6.hardware.TalonFX talonFX)
        {
            turret.getMotorController().setPosition(angle);
            talonFX.setControl(
                new com.ctre.phoenix6.controls
                    .MotionMagicVoltage(angle.in(edu.wpi.first.units.Units.Rotations))
    ...
    public LinearVelocity getFlyWheelExitSpeed(AngularVelocity velocity)
    {
        return MetersPerSecond.of(
            flyWheel.getShooterConfig()
                .getCircumference()
                .in(Meters) *Math
                .PI // This is a total fudge on the math, but it gives us a more realistic exit
    ...
    public Command getHoodSysIdCommand(GenericSubsystem launcher)
    {
        return SystemIdentification.getSysIdFullCommand(
            SystemIdentification.angleSysIdRoutine(hood.getMotorController(), hood.getName(),
                                                   launcher),
            5, 3, 3,
            ()
                -> hood.isNear(hood.getMotorController().getConfig().getMechanismUpperLimit().get(),
                               Degrees.of(10))
                       .getAsBoolean(),
            ()
                -> hood.isNear(hood.getMotorController().getConfig().getMechanismLowerLimit().get(),
                               Degrees.of(10))
                       .getAsBoolean(),
    ...
    {
        return SystemIdentification.getSysIdFullCommand(
            SystemIdentification.angleSysIdRoutine(turret.getMotorController(), turret.getName(),
                                                   launcher),
            5, 3.5, 3,
            ()
                -> turret
    ...
    public Command getHoodCharacterizationCommand(GenericSubsystem launcher)
    {
        return SystemIdentification.feedforwardCharacterization(
            launcher,
            (Voltage voltage)
                -> hood.getMotor().setVoltage(voltage),
            () -> hood.getMotorController().getMechanismVelocity().in(Degrees.per(Second)));
    }
    ...
    public Command getTurretCharacterizationCommand(GenericSubsystem launcher)
    {
        return SystemIdentification.feedforwardCharacterization(
            launcher,
            (Voltage voltage)
                -> turret.getMotor().setVoltage(voltage),
            () -> turret.getMotorController().getMechanismVelocity().in(Degrees.per(Second)));
    }
    ...
    {
        return SystemIdentification.getSysIdFullCommand(
            SystemIdentification.rpmSysIdRoutine(flyWheel.getMotorController(), flyWheel.getName(),
                                                 launcher),
            8, 3, 3);
    }
}
