...
public class TurretControlPhysics
{
     ...
     */
    public TurretControlPhysics(Translation2d turretOffsetRobotFrame,
                                ...
                                double maxRangeMeters)
    {
        ...
        SmartDashboard.putNumber("Turret Offset Angle",
                                 turretOffsetRobotFrame.getAngle().getDegrees());
    ...
    public record AimingSolution(Translation2d virtualTargetFieldPos,
     ...
     */
    public AimingSolution solve(Translation2d targetFieldPos,
                                ...
                                RobotPredictor predictor)
    {
        ...
        Rotation2d localHeading =
            fieldHeading.minus(finalState.robotStateAtFire.pose().getRotation());
        ...
        if (localHeadingRadians < minLimitRadians || localHeadingRadians > maxLimitRadians)
        {
            ...
            /**
             * If the angle is outside limits, then it clamps to a valid limit based on motion
             * direction
             */
        ...
        return new AimingSolution(finalState.virtualTargetFieldPos, fieldHeading, localHeading,
                                  feedforwardRadPerSec, distanceToTarget,
                                  finalState.requiredTimeOfFlight, status, finalState);
    }
    /** Scales feedforward when the turret is near mechanical limits */
    private double applyFeedforwardSafetyPadding(double currentAngleRadians,
                                                 double commandedFeedforward)
    {
        double minLimitRadians = minTurretAngle.getRadians();
        double maxLimitRadians = maxTurretAngle.getRadians();
     ...
     * moving
     */
    private SolverState runNewtonSolver(Translation2d targetFieldPos,
                                        Rotation2d currentTurretAngle,
                                        RobotPredictor predictor)
    {
        ...
        for (int i = 0; i < MAX_SOLVER_ITERATIONS; i++)
        {
            ...
            SolverState stateProbe =
                computePhysicsState(timeFlightGuess + DERIVATIVE_PROBE_TIME_DELTA, targetFieldPos,
                                    currentTurretAngle, predictor);

            double slope =
                (stateProbe.errorSeconds - stateCurrent.errorSeconds) / DERIVATIVE_PROBE_TIME_DELTA;

            if (Math.abs(slope) < 1e-5)
                slope = Math.signum(slope) * 1e-5;
    ...
    private SolverState computePhysicsState(double timeFlightGuess,
                                            ...
                                            RobotPredictor predictor)
    {
        ...
        Translation2d estimatedVirtualTarget = targetFieldPos.minus(
            stateNow.velocity() != null ? new Translation2d(stateNow.velocity().vxMetersPerSecond,
                                                            stateNow.velocity().vyMetersPerSecond)
                                              .times(timeFlightGuess)
                                        : new Translation2d());
        /**
         * Computes the robot heading, turret offset, and vector from the turret to the estimated
         * target
         */
        ...
        Rotation2d goalAngleLocal =
            getAngleFromVector(vectorToEstimatedTarget).minus(robotHeadingNow);
        ...
        double estimatedSettlingTime =
            settlingTimeFunction.apply(angleErrorRadians) * settlingTimeGain;
        ...
        Translation2d tangentialVelocity =
            new Translation2d(-robotAngularVelocity * turretOffsetAtFire.getY(),
        ...
        Translation2d gunPositionAtFire =
            stateAtFire.pose().getTranslation().plus(turretOffsetAtFire);
        ...
        return new SolverState(errorSeconds, requiredTimeOfFlight, virtualTargetPos,
                               vectorToVirtualTarget, inheritedMuzzleVelocity, stateAtFire, false);
    }
    ...
    private double calculateKinematicFeedforward(SolverState state)
    {
        ...
        double robotAlpha = robotState.acceleration() != null
                              ? robotState.acceleration().omegaRadiansPerSecond
                              : 0.0;

        Translation2d accelTangential = new Translation2d(-robotAlpha * turretOffsetRotated.getY(),
                                                          robotAlpha * turretOffsetRotated.getX());

        Translation2d accelCentripetal = turretOffsetRotated.times(-(robotOmega * robotOmega));
        /**
         * Converts the robot's linear acceleration into a 2D vector and defaults it to 0 if no data
         * is available
         */
        Translation2d accelRobotLinear = robotState.acceleration() != null ? new Translation2d(
                                             robotState.acceleration().vxMetersPerSecond,
                                             robotState.acceleration().vyMetersPerSecond)
                                                                           : new Translation2d();

        Translation2d accelTurretMount =
            accelRobotLinear.plus(accelTangential).plus(accelCentripetal);

        Translation2d velocityVirtualTargetDrift =
            accelTurretMount.times(-state.requiredTimeOfFlight);
        ...
        double crossProduct = (state.vectorToVirtualTarget.getX() * velocityRelative.getY())
    ...
    public record SolverState(double errorSeconds,
                              ...
                              boolean hasConverged)
    {
        ...
        {
            return new SolverState(errorSeconds, requiredTimeOfFlight, virtualTargetFieldPos,
                                   vectorToVirtualTarget, inheritedMuzzleVelocity, robotStateAtFire,
                                   true);
        }
    }
}
