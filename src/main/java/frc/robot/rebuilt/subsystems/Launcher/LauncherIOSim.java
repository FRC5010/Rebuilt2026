...
public class LauncherIOSim extends LauncherIOReal
{
    ...
    public void configureShotCalculator(ShotCalculator shotCalculator)
    {
        ...
        double minFlywheelRadPerSec = flyWheel.getShooterConfig()
                                          .getLowerSoftLimit()
                                          .orElse(RPM.of(0.0))
                                          .in(RadiansPerSecond);
        double maxFlywheelRadPerSec = flyWheel.getShooterConfig()
                                          .getUpperSoftLimit()
                                          .orElse(RPM.of(5000.0))
                                          .in(RadiansPerSecond);
        /** Reads the hood angle limits */
        Rotation2d minHoodAngle = Rotation2d.fromDegrees(
            hood.getMotorController().getConfig().getMechanismLowerLimit().get().in(Degrees));
        Rotation2d maxHoodAngle = Rotation2d.fromDegrees(
        ...
        ShotCalculator.BallisticConfig config = new ShotCalculator.BallisticConfig(
            1.0, 6.0, 0.1, minHoodAngle, maxHoodAngle, hoodStep, minFlywheelRadPerSec,
            maxFlywheelRadPerSec, wheelRadiusMeters, launchHeight, targetHeight, 0.0, 9.80665,
            Math.toRadians(90.0));
    ...
    @Override public void updateSimulation(Launcher launcher, Indexer indexer)
    {
        ...
        // Every other time this is called, determine a randome number and if > 0.5, shoot a
        // gamepiece. This would mean we try to shoot 25 times per second, and on average shoot
        // about 12-13 gamepieces per second.
                ...
                if (IntakeIOSim.intakeSimulation.obtainGamePieceFromIntake())
                {
                    ...
                    gamePieceProjectile =
                        new RebuiltFuelOnFly(
                            ...
                            Rotation2d.fromDegrees(worldPose.getRotation()
                                                       .getMeasure()
                                                       .plus(turret.getAngle())
                                                       .in(Degrees)),
                            ...
                            .withProjectileTrajectoryDisplayCallBack((pose3ds) -> {
                                Logger.recordOutput("Launcher/GamePieceTrajectory",
