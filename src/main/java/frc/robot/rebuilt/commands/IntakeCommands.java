...
public class IntakeCommands
{
    ...
    State deployed   = intakeStateMachine.addState("deployed", Commands.run(() -> {
    ...
    {
        // For auto - these get replaced with versions that take controller input instead of
        // constants
        outtaking =
            intakeStateMachine.addState("outtaking", outtakingCommand(outtakeSpeedSupplier));
        ...
        // If the hopper is not moving and we want to intake or outtake, set the hopper angle to 0
        // to prevent jamming hopperNotMoving
        ...
        deploying.switchTo(intaking).when(
            () -> hopperNotMoving.getAsBoolean() && intake.isRequested(IntakeState.INTAKING));
        ...
        deploying.switchTo(outtaking).when(
            () -> hopperNotMoving.getAsBoolean() && intake.isRequested(IntakeState.OUTTAKING));
    ...
    public void configureButtonBindings(Controller controller)
    {
        ...
        controller.setLeftTrigger(controller.createLeftTrigger().limit(
            Constants.Intake.INTAKE_MAX_IN)); // Axis are positive only hence IN
