...
public class LauncherCommands
{
    ...
    public void configureStateMachine()
    {
        ...
        autoHammerTimeState =
            stateMachine.addState("AUTO-HAMMER-TIME", autoHammerTimeStateCommand());
        ...
        idleState.switchTo(hammerTimeState)
        ...
        lowState.switchTo(hammerTimeState)
            .when(() -> launcher.isRequested(LauncherState.HAMMERTIME));
        ...
        prepState.switchTo(hammerTimeState)
            .when(() -> launcher.isRequested(LauncherState.HAMMERTIME));
        ...
        presetState.switchTo(hammerTimeState)
            .when(() -> launcher.isRequested(LauncherState.HAMMERTIME));
        ...
        escapeHammerTimeState.switchTo(idleState).when(
            () -> launcher.isRequested(LauncherState.IDLE));
        escapeHammerTimeState.switchTo(lowState).when(
            () -> launcher.isRequested(LauncherState.LOW_SPEED));
        escapeHammerTimeState.switchTo(prepState).when(
            () -> launcher.isRequested(LauncherState.PREP));

        // Hammer Time is a special case since it's a toggle state
        hammerTimeState.switchTo(lowState).when(
            () -> launcher.isRequested(LauncherState.LOW_SPEED));
        hammerTimeState.switchTo(prepState).when(() -> launcher.isRequested(LauncherState.PREP));
        hammerTimeState.switchTo(presetState)
            .when(() -> launcher.isRequested(LauncherState.PRESET));
    ...
    public void configureButtonBindings(Controller driver, Controller operator)
    {
        ...
        isTrenchTrigger.onTrue(shouldAutoHammerTimeCommand())
            .onFalse(shouldEscapeHammerTimeCommand());
    ...
    private static Command idleStateCommand()
    {
        return Commands.parallel(
            Commands.runOnce(() -> { launcher.setCurrentState(LauncherState.IDLE); }),
    ...
    {
        return Commands.parallel(Commands.runOnce(() -> {
    ...
    {
        return Commands.parallel(
            Commands.runOnce(() -> { launcher.setCurrentState(LauncherState.PRESET); }),
            Commands.run(()
                             -> launcher.usePresets(presetHoodAngle, presetTurretAngle,
                                                    presetFlywheelSpeed)));
    }
    ...
    {
        return shouldPresetCommand().andThen(Commands.runOnce(() -> {
            ShootingParameters params = launcher.getShootingParameters(
                ()
                    -> AllianceFlipUtil.apply(new Pose2d(
                        new Translation2d(
                            Inches.of(AprilTags.aprilTagFieldLayout.getTagPose(31).get().getX()
                                      + 25),
    ...
    public static Command hammerTimeStateCommand()
    {
        return Commands.parallel(Commands.run(() -> {
            ...
            launcher.usePresets(Constants.Launcher.LOW_HOOD_ANGLE, Degrees.of(0),
