...
public class TestCommands
{
    ...
    public void configureButtonBindings(Controller controller)
    {
        ...
        controller.createBButton().whileTrue(
            launcher.getTurretSysIdCommand().finallyDo(() -> launcher.stopAllMotors()));
        controller.createAButton().whileTrue(
            launcher.getFlyWheelSysIdCommand().finallyDo(() -> launcher.stopAllMotors()));

        QuestNavInterface calibrationQuest = new QuestNavInterface(new Transform3d());
        controller.createYButton().whileTrue(calibrationQuest.determineOffsetToRobotCenter(
            (GenericDrivetrain) subsystems.get(ConfigConstants.DRIVETRAIN)));

        controller.createXButton().onTrue(
            Commands.run(() -> { calibrationQuest.resetPose(new Pose3d()); }));
