...
public class ClimbCommands
{
    ...
    public static enum ClimbState { IDLE, ELEVATE, LIFTED, DESCEND, LOWERED, DISABLED, MANUAL }
    ...
    public ClimbCommands(Map<String, GenericSubsystem> systems)
    {
        ...
        idleState = stateMachine.addState(
            "idle", Commands.runOnce(() -> climb.runClimb(0))
                        .alongWith(Commands.runOnce(() -> climb.setCurrentState(ClimbState.IDLE))));
        loweredState = stateMachine.addState(
            "lowered", Commands.runOnce(() -> climb.setCurrentState(ClimbState.LOWERED)));
        liftedState = stateMachine.addState(
            "lifted", Commands.runOnce(() -> climb.setCurrentState(ClimbState.LIFTED)));
        disabledState = stateMachine.addState(
            "disabled", Commands.runOnce(() -> climb.setCurrentState(ClimbState.DISABLED)));
        manualState = stateMachine.addState(
            "manual", Commands.runOnce(() -> climb.setCurrentState(ClimbState.MANUAL))
                          .alongWith(Commands.run(() -> climb.setDefaultCommand(Commands.run(() -> {
        ...
        if (climb != null)
        {
            elevateState = stateMachine.addState(
                "elevate",
                climb.climberCommand(Meters.of(.5))
                    .alongWith(Commands.runOnce(() -> climb.setCurrentState(ClimbState.ELEVATE))));
            descendState = stateMachine.addState(
                "lower",
                climb.climberCommand(Meters.of(0))
    ...
    private void configCommonStates(Controller operator)
    {
        ...
        descendState.switchTo(loweredState)
            .when(() -> climb.getHeight().isEquivalent(Meters.of(0)));
        // elevate -> Lifted when height is =to Target
        elevateState.switchTo(liftedState)
